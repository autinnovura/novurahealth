...
const steps: Step[] = [
  ...
  {
    question: 'Which GLP-1 medication are you on?',
    type: 'select',
    options: medications,
    field: 'medication',
  },
  {
    question: 'What dose are you currently on?',
    type: 'input',
    field: 'dose',
    placeholder: 'e.g., 0.5mg, 2.5mg, not sure',
  },
  {
    question: 'When did you start your medication?',
    type: 'input',
    field: 'startDate',
    placeholder: 'e.g., January 2026, 3 months ago',
  },
  {
  ...
  {
    question: 'How much are you exercising?',
    type: 'select',
...
export default function Onboarding() {
  ...
  function advance(updatedProfile: Record<string, string>) {
    if (currentStep < steps.length - 1) {
      setCurrentStep(currentStep + 1)
    } else {
      // Save profile to localStorage and go to chat
  ...
  return (
    <div className="min-h-screen bg-[#FFFBF5] flex flex-col">
      ...
      <div className="flex-1 px-6 flex flex-col">
        <h1 className="text-2xl sm:text-3xl font-bold text-[#1E1E1C] mb-8 tracking-tight">
          {step.question}
        </h1>
        ...
        {step.type === 'input' && (
          <form onSubmit={handleInputSubmit} className="space-y-4">
            ...
            <button
              ...
              className="w-full bg-[#2D5A3D] text-white py-4 rounded-2xl text-base font-semibold hover:bg-[#3A7A52] transition-colors disabled:opacity-30 disabled:cursor-not-allowed cursor-pointer"
            >
              Continue
            </button>
          </form>
          ...
          <div className="space-y-3">
            {step.options?.map((option) => (
              <button
                ...
                className="w-full text-left px-5 py-4 rounded-2xl border-2 border-black/[0.06] bg-white text-[#2A2A28] hover:border-[#2D5A3D] hover:bg-[#E8F0EB]/30 transition-all cursor-pointer text-base"
              >
                {option}
              </button>
            ))}
        ...
        <button
          onClick={() => {
            const updated = { ...profile, [step.field]: 'Skipped' }
            setProfile(updated)
            setInputValue('')
            advance(updated)
          }}
      ...
      <div className="px-6 py-4">
        <p className="text-center text-[10px] text-[#9B9B93]">
          Your answers help Nova personalize your coaching experience. You can update these anytime.
        </p>
      </div>
    </div>
  )
